...
{
    public class FileTransferS3
    {
        string packagefolder = System.Configuration.ConfigurationManager.AppSettings["TemporaryFolder"].ToString();
        string downloadPackageFolder = System.Configuration.ConfigurationManager.AppSettings["PackageFolder"].ToString();
        int bufferLen = System.Configuration.ConfigurationManager.AppSettings["bufferLen"] != null ?
            Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["bufferLen"]):300240;
        ...
        public DataContracts.HttpFileTransferResponse DownloadFile(DataContracts.HttpFileTransferRequest request)
        {
            ...
            if (!Directory.Exists(downloadPackageFolder))
            {
                Directory.CreateDirectory(downloadPackageFolder);
            }
            string FilePath = System.IO.Path.Combine(downloadPackageFolder, request.path);
            
            if (!File.Exists(FilePath))
            {
                Constants.Log.Info("Download Packge From S3 Start");
                aWS_S3.DownloadS3ToLoacal(request.path, downloadPackageFolder);
        ...
        public string UploadFile(DataContracts.HttpFileTransferRequest request)
        {
            ...
            {
                AWS_S3 aWS_S3 = new AWS_S3();
                Constants.Log.Info("Upload Packge From S3 Start");
                aWS_S3.UploadPackageToS3(request.path, request.ReadData, request.ReadCount);
